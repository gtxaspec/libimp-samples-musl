...
static void *_dmic_record_test_thread(void *argv)
{
	...
	ret = IMP_DMIC_SetUserInfo(0, 1, 0);  /*AEC function required，Perform AEC processing on DMIC1.*/
	if (ret != 0) {
...
static void * _ao_play_thread(void *argv)
{
	...
	ret = IMP_AO_SetDigitalGain(devID, chnID, 255);
	if (ret != 0) {
		IMP_LOG_ERR(TAG, "Audio Record Set Gain failed\n");
		return NULL;
	}
...
static void *_dmic_record_test_aec_thread(void *argv)
{
	...
	/* Step 1: set dmic user info:if need aec function*/
	ret = IMP_DMIC_SetUserInfo(0, 1, 1);  /*AEC function required，Perform AEC processing on DMIC1*/
	if (ret != 0) {
		IMP_LOG_ERR(TAG, "dmic set user info error.\n");
		return NULL;
	}

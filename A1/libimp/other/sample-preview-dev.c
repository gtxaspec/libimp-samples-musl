...
static IMP_S32 g_s32MaxVdecChn;
static IMP_BOOL g_bUserPool = IMP_FALSE;
static IMP_S32 g_s32LastPreviewMode;
static IMP_U32 g_u32PoolId[MAX_VDEC_DEVS];
static pthread_t g_sendTid[MAX_VDEC_DEVS];
static SAMPLE_VDEC_THREAD_S g_stVdecThreadPara[MAX_VDEC_DEVS];
...
IMP_S32 sample_preview_switch(SAMPLE_VO_MODE_E enVoMode)
{
	...
	VDEC_DEV_ATTR_S stOldVdecDevAttr;
	VDEC_DEV_ATTR_S stNewVdecDevAttr;
	VDEC_CHN_ATTR_S stVdecChnAttr;
	VDEC_BUFFER_PARAM_S stVdecBufferParam;
	IMP_U32 u32FrameBufferCnt = 3;
	char s8Name[64];

	memset(&stVdecChnAttr,0,sizeof(VDEC_CHN_ATTR_S));
	memset(&stVdecBufferParam,0,sizeof(VDEC_BUFFER_PARAM_S));
	...
	if (u32WndNum > g_s32MaxVdecChn) {
		IMP_LOG_ERR(TAG,"window num bigger than Max chn num %d!\n", g_s32MaxVdecChn);
		return IMP_FAILURE;
	}

	IMP_VDEC_GetDevAttr(0, &stOldVdecDevAttr);
	for (i = 0; i < g_s32LastPreviewMode; i++){
		g_stVdecThreadPara[i].enCtrlSig = VDEC_CTRL_STOP;
		pthread_join(g_sendTid[i],IMP_NULL);
		sample_vdec_stop_recv_stream(i);
		sample_vdec_stop_chn(i,s32VdecChn);
		sample_vdec_stop_dev(i);
		if (g_bUserPool) {
			IMP_VDEC_DetachVbPool(i);
			IMP_System_DestroyPool(g_u32PoolId[i], NULL);
		}
	...
	for (i = 0; i < u32WndNum; i++){
		stNewVdecDevAttr.enType = stOldVdecDevAttr.enType;
		stNewVdecDevAttr.enMode = VIDEO_MODE_FRAME;
		stNewVdecDevAttr.u32PicWidth = stOldVdecDevAttr.u32PicWidth;
		stNewVdecDevAttr.u32PicHeight = stOldVdecDevAttr.u32PicHeight;
		stNewVdecDevAttr.u32StreamBufSize = stOldVdecDevAttr.u32PicWidth * stOldVdecDevAttr.u32PicHeight * 3 / 4;
		switch(stNewVdecDevAttr.enType){
			case PT_H264:
				stNewVdecDevAttr.u32RefFrameNum = 2;
				break;
			case PT_H265:
				stNewVdecDevAttr.u32RefFrameNum = 2;
				break;
			case PT_JPEG:
				stNewVdecDevAttr.u32RefFrameNum = 0;
				break;
			default:
				IMP_LOG_ERR(TAG, "stream type error\n");
				return IMP_FAILURE;
		}
		stNewVoChnAttr.stRect.s32X       = ALIGN_BACK(ALIGN_BACK(u32Width/u32Square,2) * (i%u32Square), 2);
		stNewVoChnAttr.stRect.s32Y       = ALIGN_BACK((u32Height/u32Square) * (i/u32Square), 2);
		...
		if (g_bUserPool) {
			memcpy(&stVdecBufferParam.stDevAttr, &stNewVdecDevAttr, sizeof(VDEC_DEV_ATTR_S));
			stVdecBufferParam.bEnableChn0 = IMP_TRUE;
			stVdecBufferParam.stChn0Attr.enOutputFormat = PIX_FMT_NV12;
			stVdecBufferParam.stChn0Attr.u32OutputWidth = ALIGN_UP(stNewVoChnAttr.stRect.u32Width,8);
			stVdecBufferParam.stChn0Attr.u32OutputHeight = stNewVoChnAttr.stRect.u32Height;
			stVdecBufferParam.stChn0Attr.u32FrameBufCnt = u32FrameBufferCnt;
			stVdecBufferParam.bEnableChn1 = IMP_FALSE;

			sprintf(s8Name, "Dev%d-pool", i);
			s32Ret = IMP_System_CreatPool(g_u32PoolId + i, IMP_VDEC_GetBufferSize(&stVdecBufferParam), 1, s8Name);
			if (s32Ret != IMP_SUCCESS) {
				IMP_LOG_ERR(TAG, "IMP_System_CreatPool failed! ret = 0x%#x\n", s32Ret);
				return s32Ret;
			}

			s32Ret = IMP_VDEC_AttachVbPool(i, g_u32PoolId[i]);
			if (s32Ret != IMP_SUCCESS) {
				IMP_LOG_ERR(TAG, "IMP_VDEC_AttachVbPool failed! ret = 0x%#x\n", s32Ret);
				return s32Ret;
			}
		}

		s32Ret = sample_vdec_start_dev(i, &stNewVdecDevAttr);
		if (s32Ret != IMP_SUCCESS) {
		...
		stVdecChnAttr.u32FrameBufCnt = u32FrameBufferCnt;
		...
		if (s32Ret != IMP_SUCCESS) {
			IMP_LOG_ERR(TAG, "sample_vdec_start_chn failed! ret = 0x%#x\n", s32Ret);
			return s32Ret;
		}
		g_stVdecThreadPara[i].enCtrlSig = VDEC_CTRL_START;
		pthread_create(&g_sendTid[i], 0, sample_vdec_send_stream_thread, (void *)&g_stVdecThreadPara[i]);
	}
	g_s32LastPreviewMode = enVoMode;
	IMP_LOG_INFO(TAG,"preview mode =%d u32WndNum=%d\n", g_s32LastPreviewMode,u32WndNum);
	return IMP_SUCCESS;
}
...
IMP_S32 sample_preview_vdec_open(IMP_S32 s32PrevWin,PAYLOAD_TYPE_E enType,IMP_U32 u32Width,IMP_U32 u32Height)
{
	...
	if (g_bUserPool) {
		memcpy(&stVdecBufferParam.stDevAttr, &stVdecDevAttr, sizeof(VDEC_DEV_ATTR_S));
		stVdecBufferParam.bEnableChn0 = IMP_TRUE;
		stVdecBufferParam.stChn0Attr.enOutputFormat = PIX_FMT_NV12;
		stVdecBufferParam.stChn0Attr.u32OutputWidth = ALIGN_UP(stVoChnAttr.stRect.u32Width,8);
		stVdecBufferParam.stChn0Attr.u32OutputHeight = stVoChnAttr.stRect.u32Height;
		stVdecBufferParam.stChn0Attr.u32FrameBufCnt = u32FrameBufferCnt;
		stVdecBufferParam.bEnableChn1 = IMP_FALSE;

		sprintf(s8Name, "Dev%d-pool", s32PrevWin);
		s32Ret = IMP_System_CreatPool(g_u32PoolId + s32PrevWin, IMP_VDEC_GetBufferSize(&stVdecBufferParam), 1, s8Name);
		if (s32Ret != IMP_SUCCESS) {
			IMP_LOG_ERR(TAG, "IMP_System_CreatPool failed! ret = 0x%08x\n", s32Ret);
			return s32Ret;
		}

		s32Ret = IMP_VDEC_AttachVbPool(s32PrevWin, g_u32PoolId[s32PrevWin]);
		if (s32Ret != IMP_SUCCESS) {
...
END0:
	...
	if (g_bUserPool) {
		IMP_VDEC_DetachVbPool(s32PrevWin);
		IMP_System_DestroyPool(g_u32PoolId[s32PrevWin], NULL);
	}
	return IMP_SUCCESS;
}
...
/*
   sample-preview-dev和sample-preview的区别是：sample-preview在切屏
   时仅仅重建vdec channel而不重建vdec device；而sample-preview-dev
   会将vdec device和channel全部重建。建议使用全部重建的方式，这样可
   以减少内存碎片。
 */
int main(int argc,char *argv[])
{
    ...
    *************************************************/
	for(i = 0; i < u32VdecMux; i++){
		...
		g_stVdecThreadPara[i].s32DevId = i;
		sprintf(g_stVdecThreadPara[i].cFileName,DECODE_FILE);
		g_stVdecThreadPara[i].enType = PT_H265;
		g_stVdecThreadPara[i].enVideoMode = VIDEO_MODE_FRAME;
		g_stVdecThreadPara[i].u32BufSize = 640 * 360 * 3 / 4;
		g_stVdecThreadPara[i].enCtrlSig = VDEC_CTRL_START;
		g_stVdecThreadPara[i].s32FpsCtrl = 25;
		g_stVdecThreadPara[i].bLoopPlayback = IMP_TRUE;
		pthread_create(&g_sendTid[i], 0, sample_vdec_send_stream_thread, (void *)&g_stVdecThreadPara[i]);
	}
	g_s32MaxVdecChn = u32VdecMux;
	...
	while(1){
		...
		if (strncmp("help", Cmd, strlen("help")) == 0) {
			...
			sample_preview_switch(VO_MODE_1MUX);
		}else if(strncmp("4mux", Cmd, strlen("4mux")) == 0){
			sample_preview_switch(VO_MODE_4MUX);
		}else if(strncmp("9mux", Cmd, strlen("9mux")) == 0){
			sample_preview_switch(VO_MODE_9MUX);
		}else if(strncmp("16mux", Cmd, strlen("16mux")) == 0){
			sample_preview_switch(VO_MODE_16MUX);
		}else if(strncmp("25mux", Cmd, strlen("25mux")) == 0){
			sample_preview_switch(VO_MODE_25MUX);
		}else if(strncmp("36mux", Cmd, strlen("36mux")) == 0){
			sample_preview_switch(VO_MODE_36MUX);
...
END2:
	...
	sample_disable_vo_all(g_s32LastPreviewMode);
END1:
	for(i = 0; i < u32VdecMux; i++){
		stVdecCell.enModId = MOD_ID_VDEC;
		stVdecCell.u32DevId = i;

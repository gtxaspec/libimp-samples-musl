...
/* System init */
IMPSensorInfo sensor_info1;
static int my_sample_system_init()
{
	int ret = 0;

	memset(&sensor_info1, 0, sizeof(IMPSensorInfo));
	memcpy(sensor_info1.name, SENSOR_NAME, sizeof(SENSOR_NAME));
	sensor_info1.cbus_type = SENSOR_CUBS_TYPE;
	memcpy(sensor_info1.i2c.type, SENSOR_NAME, sizeof(SENSOR_NAME));
	sensor_info1.i2c.addr = SENSOR_I2C_ADDR;

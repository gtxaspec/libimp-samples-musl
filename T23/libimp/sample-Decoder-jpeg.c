...
#define TAG		"JPEGD"
#define JPEGFILENAME		"1920x1080.jpeg_1"
...
retry_pooling_point:
	...
	ret = write(videofd, (void *)frame->virAddr, frame->width * frame->height * 3 / 2);
	if (ret != frame->width * frame->height * 3 / 2) {
		IMP_LOG_ERR(TAG, "IMP_Decoder_GetFrame(%d) failed\n", 0);
		goto err_write_frame;
	}

	IMP_LOG_DBG(TAG, "frame->width=%u, frame->height=%u, frame->size=%u\n", frame->width, frame->height, frame->size);

...
static resolution_size_t sensor_resolution[] = {
	...
	[SENSOR_SC2235] = {1920, 1080},
	[SENSOR_IMX307] = {1920, 1080},
	[SENSOR_OV4689] = {2592, 1520},
};
...
int sample_res_get_sensor_type(const char *sensor_type_string, sensor_type_t *sensor_type)
{
	...
	} else if (!strncmp(sensor_type_string, "sc2235", sizeof("sc2235"))) {
		*sensor_type = SENSOR_SC2235;
	} else if (!strncmp(sensor_type_string, "imx307", sizeof("imx307"))) {
		*sensor_type = SENSOR_IMX307;
	} else if (!strncmp(sensor_type_string, "ov4689", sizeof("ov4689"))) {
		*sensor_type = SENSOR_OV4689;
	} else {
...
int sample_res_get_sensor_info(sensor_type_t sensor_type, IMPSensorInfo *sensor_info)
{
	...
	switch (sensor_type) {
		...
		case SENSOR_SC2235:
			...
			break;
		case SENSOR_IMX307:
			memcpy(sensor_info->name, "imx307", sizeof("imx307"));
			sensor_info->cbus_type = TX_SENSOR_CONTROL_INTERFACE_I2C;
			memcpy(sensor_info->i2c.type, "imx307", sizeof("imx307"));
			sensor_info->i2c.addr = 0x1a;
			break;
		case SENSOR_OV4689:
			memcpy(sensor_info->name, "ov4689", sizeof("ov4689"));
			sensor_info->cbus_type = TX_SENSOR_CONTROL_INTERFACE_I2C;
			memcpy(sensor_info->i2c.type, "ov4689", sizeof("ov4689"));
			sensor_info->i2c.addr = 0x36;
			break;
		default:
		...
		int i_fps_num, int i_fps_den, int vbnum)
{
	...
	int enable_scaler = 0;
	int scaler_width = sensor_resolution[sensor_type].width;
	int scaler_height = sensor_resolution[sensor_type].height;
	...
	fschn_attr->scaler.outwidth = scaler_width;
	fschn_attr->scaler.outheight = scaler_height;

	fschn_attr->crop.enable = 1;
	fschn_attr->crop.top = 0;
	fschn_attr->crop.left = 0;
	fschn_attr->crop.width = size.width;
	fschn_attr->crop.height = size.height;
		...
		resolution_size_t size, int outFrmRateNum, int outFrmRateDen, int outBitRate)
{
	...
	if (type == PT_H264 && ((mode < ENC_RC_MODE_FIXQP) || (mode > ENC_RC_MODE_SMART))) {
	...
	if (type == PT_H264) {
        rcAttr->outFrmRate.frmRateNum = outFrmRateNum;
        rcAttr->outFrmRate.frmRateDen = outFrmRateDen;
        rcAttr->maxGop = outFrmRateNum * 2 / outFrmRateDen;
        if (mode == ENC_RC_MODE_CBR) {
            rcAttr->attrRcMode.rcMode = ENC_RC_MODE_CBR;
            rcAttr->attrRcMode.attrH264Cbr.outBitRate = outBitRate;
            ...
            rcAttr->attrRcMode.attrH264Vbr.maxBitRate = outBitRate;
            ...
            rcAttr->attrHSkip.hSkipAttr.bBlackEnhance = 0;
            rcAttr->attrHSkip.maxHSkipType = IMP_Encoder_STYPE_N1X;
        } else if (mode == ENC_RC_MODE_SMART) {
            ...
            rcAttr->attrRcMode.attrH264Smart.maxBitRate = outBitRate;

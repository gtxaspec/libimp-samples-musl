...
{
	/*The width and height of the pattern used in the sample is 64.*/
...
static void draw_narmal(IMPOsdRgnType type)
{
	...
	if(OSD_REG_ISP_LINE_RECT == type)
	{
		...
		rIspOsdAttr.osdispdraw.stDrawAttr.cfg.line.startx = 200;    /*Draw vertical line.*/
...
{
	/*Draw Line, Box, and Rectangle Mask*/
	draw_narmal(OSD_REG_ISP_LINE_RECT);

	IMP_LOG_DBG(TAG,"ISP OSD draw_narmal success\n");

	/*Draw image on OSD requires using different ISP API compared to drawing lines, boxes, and rectangles.*/
	draw_pic();
...
#ifdef ISPOSD
	sample_osd_exit_isp();//Destroy the timestamp region of the main stream.

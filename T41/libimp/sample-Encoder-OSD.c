 ...
 *Detailed explanations for all the called APIs can be found in the header files under the directory "proj/sdk-lv3/include/api/cn/imp/".
 *This sample supports up to three data streams, with the main stream using ISP to draw OSD (supporting direct output), the secondary stream using IPU to draw OSD, 
 *and the third stream not drawing OSD. This sample is only intended as an example for using direct output, ISPOSD, and IPUOSD.
* */
...
static void draw_narmal(IMPOsdRgnType type)
{
	...
	if(OSD_REG_ISP_LINE_RECT == type)
	{
		...
		rIspOsdAttr.osdispdraw.stDrawAttr.cfg.line.alpha = 4; /*The range is [0,4], and the smaller the value, the more transparent it is.*/
...
void* isp_osd_thread(void *arg)
{
	...
	/*Drawing lines, boxes, and rectangles to cover the image*/
	draw_narmal(OSD_REG_ISP_LINE_RECT);

	IMP_LOG_INFO(TAG,"ISP OSD draw_narmal success\n");

	/*Drawing an image requires a different ISP interface compared to drawing lines, rectangles, and boxes for masking.*/
...
IMPRgnHandle *sample_ipuosd_init(int grpNum)
{
	...
	//Actually, the value of grAttrFont.show is what we configure for ourselves.
...
int main(int argc, char *argv[])
{
	...
	/* Step.5 Bind */
	for (i = 0; i < FS_CHN_NUM; i++) {
		/*All streams except the sub stream can use ISPOSD or direct encoding. The sub stream runs IPUOSD and needs to bind to the OSD module.*/
		if (chn[i].enable && 1 != i) {

...
static int sample_osd_exalgo_start(int grp_num)
{
    ...
    for (i = 0; i < MAX_EXALGO_RECTNUM; i++) {
        ...
        grAttrRect.show = 0;
        grAttrRect.layer = i;
        grAttrRect.scalex = SENSOR_WIDTH / SENSOR_WIDTH_THIRD;
        grAttrRect.scaley = SENSOR_HEIGHT / SENSOR_HEIGHT_THIRD;
...
static int sample_ivs_exalgo_start(int grp_num, int chn_num, IMPIVSInterface **interface)
{
	...
	param.width = SENSOR_WIDTH_THIRD;
	param.height = SENSOR_HEIGHT_THIRD;
	...
	for (i = 0; i < NR_FRAMES_TO_SAVE; i++) {
        ...
        if (result->rectCnt > 0) {
            ...
            for (j = 0; j < result->rectCnt; j++) {
                ...
                //IMP_LOG_INFO(TAG, "result=%p[%d]:((%d,%d),(%d,%d))\n", result, i, x0, y0, x1, y1);
            }
...
int main(int argc, char *argv[])
{
    ...
    ret = IMP_System_Bind(&chn[1].framesource_chn, &ivscell);
    if (ret < 0) {
        IMP_LOG_ERR(TAG, "Bind FrameSource channel1 and ivs failed\n");
        return -1;
    }
    ret = IMP_System_Bind(&ivscell, &chn[1].imp_encoder);
    if (ret < 0) {
    ...
    ret = IMP_System_UnBind(&chn[1].framesource_chn, &ivscell);
    if (ret < 0) {
        IMP_LOG_ERR(TAG, "UnBind FrameSource channel1 and ivs failed\n");
        return -1;
    }
    ret = IMP_System_UnBind(&ivscell, &chn[1].imp_encoder);
    if (ret < 0) {

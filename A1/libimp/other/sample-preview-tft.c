...
IMP_VOID sample_vdec_handle_sig(IMP_S32 s32Signo)
{
	IMP_S32 s32Ret = 0;
	...
	if (s32Ret != IMP_SUCCESS) {
		IMP_LOG_ERR(TAG, "sample_system_exit failed! ret = 0x%08x\n", s32Ret);
		return;
	}
	exit(0);
}
...
IMP_S32 sample_enable_vo_all(SAMPLE_VO_MODE_E enVoMode)
{
	...
	stVoPubAttr.stCanvasSize.u32Width = 1024;
	stVoPubAttr.stCanvasSize.u32Height = 600;
	stVoPubAttr.enIntfType = VO_INTF_TFT;
	stVoPubAttr.enIntfSync = VO_OUTPUT_USER;
	stVoPubAttr.stSyncInfo.bSyncMode = 1;
	stVoPubAttr.stSyncInfo.bIop = 0;
	stVoPubAttr.stSyncInfo.u8Intfb = 0;
	stVoPubAttr.stSyncInfo.u16Freq = 60;
	stVoPubAttr.stSyncInfo.u16Vact = 600;
	stVoPubAttr.stSyncInfo.u16Vfp = 12;//------>
	stVoPubAttr.stSyncInfo.u16Vbp = 20;//fix 23
	stVoPubAttr.stSyncInfo.u16Vpw = 3;
	stVoPubAttr.stSyncInfo.u16Hact = 1024;
	stVoPubAttr.stSyncInfo.u16Hfp = 160;//-------->
	stVoPubAttr.stSyncInfo.u16Hbp = 140;//fix 160
	stVoPubAttr.stSyncInfo.u16Hpw = 20;
	stVoPubAttr.stSyncInfo.bIhs = 0;
	stVoPubAttr.stSyncInfo.bIvs = 1;
...
#if 0
	s32Ret = sample_vo_hdmi_start(VO_OUTPUT_1024x768_60);
	if(s32Ret != IMP_SUCCESS){
	...
	s32VoLayer = SAMPLE_VO_LATER0;
	stVoLayerAttr.enPixFmt = PIX_FMT_NV12;
	stVoLayerAttr.stDispRect.u32Width = stVoPubAttr.stSyncInfo.u16Hact;
	stVoLayerAttr.stDispRect.u32Height = stVoPubAttr.stSyncInfo.u16Vact;
